...
void AllocatorTest::HashSegTest() {
    void* ptr = aloc.MmapAlloc(PageSizeToBytes(aloc.kHashSegPageSize), node, filename, linenum);
    ...
    ptr = aloc.MmapAlloc(PageSizeToBytes(aloc.kHashSegPageSize), node, filename, linenum);
    EXPECT_TRUE(ptr != nullptr);
    EXPECT_EQ(0, aloc.reserved_hash_segs.size());
    aloc.MmapFree(ptr);
    EXPECT_EQ(0, aloc.AllocatedPages());
...
void AllocatorTest::BuddyFixedSizeTest() {
    ...
    // for (int size = 64, order = 6; size < kBuddyHeapSize; size <<= 1, order++) {
        ...
        for (int i = 1; i < kBuddyHeapSize / size; i++) {
            ptrs.push_back(aloc.MmapAlloc(PageSizeToBytes(size), node, filename, linenum));
...
void AllocatorTest::BuddyIncreasingFillTest() {
    ...
    for (int p_size : {1, 1, 2, 3, 5, 8, 13, 21}) {
        ptrs.push_back(aloc.MmapAlloc(PageSizeToBytes(p_size), node, filename, linenum));
        EXPECT_TRUE(ptrs.back() != nullptr);
...
void AllocatorTest::BstAllocateTest() {
    vector<void*> ptrs;
    for (int i = 1; i <= 3; i++) {
        ptrs.push_back(aloc.MmapAlloc(PageSizeToBytes(kBuddyHeapSize + i * 10), node, filename, linenum));
        EXPECT_TRUE(ptrs.back() != nullptr);
...
void AllocatorTest::HybridAllocateTest() {
    vector<void*> ptrs;
    ptrs.push_back(aloc.MmapAlloc(PageSizeToBytes(kBuddyHeapSize - 1), node, filename, linenum));
